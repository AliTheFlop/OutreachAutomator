...
class ApiService {
  ...
  async createTemplate(template: Omit<ApiTemplate, 'id' | 'createdAt'>): Promise<string> {
    const response = await fetch(`${API_BASE}/templates`, {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
  ...
  async createCampaign(campaign: Omit<ApiCampaign, 'id' | 'status' | 'sentCount' | 'openCount' | 'clickCount' | 'createdAt' | 'scheduledAt'>): Promise<string> {
    const response = await fetch(`${API_BASE}/campaigns`, {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
  ...
  async updateCampaignStatus(id: string, status: ApiCampaign['status']): Promise<void> {
